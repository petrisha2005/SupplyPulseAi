...
const categoryChannelWeights = (category, index) => {
    const base = Object.fromEntries(channels.map((channel) => [channel, 0]));
...
const supplierIdsForCategory = (category) => {
...
const splitStock = (total, index) => {
    ...
    const channelStock = Object.fromEntries(channels.map((channel) => [channel, Math.max(0, Math.round(total * weights[channel] / sum))]));
...
for (const [skuIndex, sku] of skus.entries()) {
    ...
    const channelWeights = Object.fromEntries(channels.map((channel) => [channel, rawChannelWeights[channel] / channelWeightTotal]));
    ...
    for (let dayOffset = -89; dayOffset <= 0; dayOffset += 1) {
        ...
        for (const channel of channels.filter((item) => item !== "ERP")) {
            const channelSpike = channel === "Meesho" && skuIndex % 10 === 0 && dayOffset >= -8 ? 1.45 :
