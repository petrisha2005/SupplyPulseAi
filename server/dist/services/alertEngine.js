...
const alertKey = (alert) => [alert.type, alert.skuId ?? "", alert.supplierId ?? "", alert.channel ?? "", alert.source ?? ""].join("|");
...
const upsertAlert = (input) => {
    const now = nowIso();
    const skuId = input.skuId ?? input.sku ?? "";
    const severity = normalizeSeverity(input.severity);
    const existing = dataStore.getAlerts().find((alert) => (alert.status ?? "Pending") === "Pending" &&
...
export const createAlert = (input) => upsertAlert(input);
export const generateAlerts = () => {
    ...
    for (const sku of risks.slice(0, 60)) {
        ...
        const channelEntries = Object.entries(sku.channelStock);
        ...
        const concentrated = Object.entries(sku.channelDemandSplit).sort((a, b) => b[1] - a[1])[0];
...
export const buildAlerts = (filters = []) => {
    generateAlerts();
...
export const getAlerts = (filters = {}) => {
    ...
    if (filters.severity)
        alerts = alerts.filter((alert) => String(alert.severity) === filters.severity);
    if (filters.status)
        alerts = alerts.filter((alert) => (alert.status ?? "Pending") === filters.status);
    if (filters.type)
        alerts = alerts.filter((alert) => alert.type === filters.type);
    if (filters.skuId)
        alerts = alerts.filter((alert) => alert.skuId === filters.skuId || alert.sku === filters.skuId);
    if (filters.supplierId)
        alerts = alerts.filter((alert) => alert.supplierId === filters.supplierId);
    alerts = alerts.sort((a, b) => (severityRank[String(b.severity)] ?? 0) - (severityRank[String(a.severity)] ?? 0) ||
...
export const createPipelineAlert = (runId, alertsGenerated) => upsertAlert({
