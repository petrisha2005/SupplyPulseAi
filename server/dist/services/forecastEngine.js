...
const channels = () => dataStore.getChannels().filter((channel) => publicMarketplaceChannels.has(channel));
const netUnits = (record) => Math.max(0, record.unitsSold - record.returns);
const recordsInWindow = (records, days) => {
    const start = addDays(DEMO_TODAY, -(days - 1));
    return records.filter((record) => record.date >= start && record.date <= DEMO_TODAY);
};
const sumUnits = (records) => records.reduce((sum, record) => sum + netUnits(record), 0);
const averageUnits = (records, days) => round(sumUnits(records) / Math.max(1, days), 2);
const dateKey = (date) => new Date(`${date}T00:00:00.000Z`).getUTCDay();
const groupDailySales = (records) => {
...
const upcomingEventForSku = (sku) => {
    ...
    if (simulated && (!seeded || simulated.demandMultiplier >= seeded.demandMultiplier))
        return simulated;
    return seeded;
};
const eventMultiplierForDay = (event, forecastDate) => {
    if (!event)
        return 1;
    const daysUntilStart = daysBetween(DEMO_TODAY, event.startDate);
    const daysFromStart = daysBetween(event.startDate, forecastDate);
    if (daysFromStart >= 0 && forecastDate <= event.endDate)
...
const trendDirection = (trendRatio) => {
    if (trendRatio >= 1.08)
        return "Rising";
    if (trendRatio <= 0.92)
        return "Falling";
...
const confidenceReasons = ({ score, completeness, volatilityRatio, trendRatio, event }) => {
    const reasons = [
    ...
    if (event)
        reasons.push(`${event.name} adds event uncertainty for affected channels.`);
    reasons.push(`Overall confidence is ${confidenceLabel(score).toLowerCase()} at ${score}%.`);
    return reasons;
};
const calculateConfidence = ({ historyDays, last28DailyUnits, trendRatio, event }) => {
...
const channelDemandSplit = (records, baselineRecords) => {
    const recentByChannel = Object.fromEntries(channels().map((channel) => [channel, 0]));
    const baselineByChannel = Object.fromEntries(channels().map((channel) => [channel, 0]));
    for (const record of records)
        recentByChannel[record.channel] += netUnits(record);
    for (const record of baselineRecords)
        baselineByChannel[record.channel] += netUnits(record);
...
const sameWeekdayAverage = (dailyHistory, targetDate, fallback) => {
...
export const getForecastForSku = (skuId) => {
    const cached = getCache(`forecast:${skuId}`);
    if (cached)
        return cached;
...
const toForecastSummaryItem = (forecast) => {
    const { historicalDailySales: _historicalDailySales, forecastNext7Days: _forecastNext7Days, forecastNext30Days: _forecastNext30Days, historical: _historical, next7DaysDemand: _next7DaysDemand, next30DaysDemand: _next30DaysDemand, revenueAtRiskSeries: _revenueAtRiskSeries, forecastExplanation: _forecastExplanation, confidenceReasons: _confidenceReasons, ...summary } = forecast;
    return summary;
};
export const getForecastSummary = () => {
    const cached = getCache("forecast:summary");
    if (cached)
        return cached;
...
export const compareForecasts = (skuIds) => ({
    skuIds,
    forecasts: skuIds
