...
const indexSalesHistory = (records) => {
    const bySku = new Map();
...
const loadSeedState = () => {
    const salesHistory = readJson("salesHistory.json");
    return {
        skus: readJson("skus.json"),
        suppliers: readJson("suppliers.json"),
...
export const dataStore = {
    ...
    getSkuById: (skuId) => state.skus.find((sku) => sku.skuId === skuId),
    getAllSuppliers: () => state.suppliers,
    getSupplierById: (supplierId) => state.suppliers.find((supplier) => supplier.supplierId === supplierId),
    getAllEvents: () => state.events,
    getChannels: () => state.channels,
    getSalesHistory: (skuId) => skuId ? state.salesHistoryBySku.get(skuId) ?? [] : state.salesHistory,
    updateSkuStock: (skuId, changes) => {
        const sku = state.skus.find((item) => item.skuId === skuId);
        if (!sku)
            return undefined;
        if (typeof changes.currentStock === "number")
            sku.currentStock = Math.max(0, changes.currentStock);
        if (typeof changes.committedStock === "number")
            sku.committedStock = Math.max(0, changes.committedStock);
    ...
    updateSupplierDelay: (supplierId, delayDays) => {
