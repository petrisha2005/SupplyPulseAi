...
const sumUnits = (records) => records.reduce((sum, record) => sum + Math.max(0, record.unitsSold - record.returns), 0);
const unitsForChannel = (records, channel) => records.filter((record) => record.channel === channel).reduce((sum, record) => sum + Math.max(0, record.unitsSold - record.returns), 0);
const channelSplit = (records) => {
    const channels = dataStore.getChannels();
    const totals = Object.fromEntries(channels.map((channel) => [channel, 0]));
    for (const record of records)
        totals[record.channel] += Math.max(0, record.unitsSold - record.returns);
    const total = Object.values(totals).reduce((sum, value) => sum + value, 0) || 1;
    return Object.fromEntries(channels.map((channel) => [channel, round(totals[channel] / total, 2)]));
};
const upcomingEvent = (sku) => {
...
const formatDateLabel = (dateString) => new Intl.DateTimeFormat("en-IN", {
    day: "2-digit",
    month: "short",
    year: "numeric",
    timeZone: "UTC"
}).format(new Date(`${dateString}T00:00:00.000Z`));
const stockoutLabel = (daysOfCover, date) => {
    if (daysOfCover < 1)
        return "Within 24 hours";
    if (daysOfCover < 2)
        return "Tomorrow";
    if (daysOfCover >= 31)
        return `Safe for ${Math.floor(daysOfCover)} days`;
...
const calculateFormula = ({ daysOfCover, leadTime, velocityTrend, event, demandMultiplier, supplierReliability, supplierDelayDays, committedStock, totalAvailableStock, channelConcentration }) => {
    const leadGap = Math.max(0, leadTime - daysOfCover);
    const daysCoverRaw = daysOfCover <= 2 ? 100 :
...
const totalFormulaScore = (breakdown) => Math.round(breakdown.daysCoverRisk +
...
const supplierRawRecord = (supplierId) => dataStore.getSupplierById(supplierId);
const publicMarketplaceChannels = new Set(["Amazon", "Shopify", "Meesho", "Flipkart"]);
export const toRiskListSku = (sku) => {
    ...
    const topChannels = Object.entries(sku.channelDemandSplit)
...
export const toInventoryListSku = (sku) => ({
    skuId: sku.skuId,
...
export const enrichSku = (sku) => {
    ...
    const channelHotspot = channelEntries[0][0];
    ...
    const lowestInventoryChannel = Object.entries(sku.channelStock)
...
export const getInventory = () => {
    const cached = getCache("risk:inventory");
    if (cached)
        return cached;
...
export const getRiskBySku = (skuId) => getInventory().find((sku) => sku.skuId === skuId);
export const getRiskExplanationBySku = (skuId) => getRiskBySku(skuId);
