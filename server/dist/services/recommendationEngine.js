...
import { getRisks, getRiskBySku } from "./riskEngine.js";
const urgencyRank = {
    Immediate: 4,
    "Within 24 hours": 3,
    "This week": 2,
...
const deadlineLabel = (urgency) => {
    if (urgency === "Immediate")
        return "Today before 6 PM";
    if (urgency === "Within 24 hours")
        return "Within 24 hours";
    if (urgency === "This week")
        return "Before supplier call tomorrow";
    return "Monitor only";
};
const alertSeverity = (urgency, riskLevel) => {
    if (urgency === "Immediate" || riskLevel === "Critical")
        return "critical";
    if (urgency === "Within 24 hours" || riskLevel === "High")
        return "warning";
    return "info";
};
const urgencyFromSku = (sku) => {
    if (sku.daysOfCover <= 2 || sku.riskLevel === "Critical")
        return "Immediate";
    if (sku.daysOfCover <= sku.leadTime)
        return "Within 24 hours";
    if (sku.riskLevel === "High")
        return "This week";
    return "Monitor";
};
const legacyUrgencyLevel = (urgency, riskLevel) => {
    if (urgency === "Immediate")
        return "Critical";
    if (urgency === "Within 24 hours")
        return "High";
...
const supplierUnitCost = (sku, supplier) => {
...
const supplierScore = (sku, supplier, urgency) => {
...
const supplierReason = (supplier, urgency, rank) => {
    if (urgency === "Immediate")
        return `${rank === 0 ? "Chosen" : "Backup"} for fastest reliable dispatch: ${supplier.avgLeadDays} day lead time, ${supplier.onTimeDeliveryPct}% on-time delivery.`;
    return `${rank === 0 ? "Chosen" : "Backup"} for balanced reliability, lead time, and cost rating ${supplier.costRating}/5.`;
};
const rankSuppliers = (sku, urgency) => {
...
const roundToMinOrder = (quantity, minOrder) => Math.ceil(quantity / Math.max(1, minOrder)) * Math.max(1, minOrder);
const capQuantity = (quantity, avgDailyForecast) => {
    const sensibleCap = Math.max(250, Math.ceil(avgDailyForecast * 60));
    return Math.min(quantity, sensibleCap);
};
const calculateQuantity = (sku, supplier, urgency, forecast) => {
    ...
    if (quantity <= 0 && ["Immediate", "Within 24 hours", "This week"].includes(urgency ?? ""))
        quantity = Math.max(sku.safetyStock, supplier.minOrderQuantity ?? 1);
...
const deadlineDate = (urgency) => {
    if (urgency === "Immediate")
        return DEMO_TODAY;
    if (urgency === "Within 24 hours")
        return addDays(DEMO_TODAY, 1);
    if (urgency === "This week")
        return addDays(DEMO_TODAY, 3);
...
const publicMarketplaceChannels = new Set(["Amazon", "Shopify", "Meesho", "Flipkart"]);
const channelSignal = (sku) => {
    const highestSales = Object.entries(sku.channelDemandSplit).filter(([channel]) => publicMarketplaceChannels.has(channel)).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "Amazon";
    const lowestInventory = Object.entries(sku.channelStock).filter(([channel, stock]) => publicMarketplaceChannels.has(channel) && Number(stock) > 0).sort((a, b) => Number(a[1]) - Number(b[1]))[0]?.[0] ?? highestSales;
    return { highestSales, lowestInventory };
};
const buildRecommendation = (sku) => {
    ...
    if (supplierDelayWarning)
        reasonBullets.push("Supplier has recent delay risk. Consider alternate supplier if dispatch is not confirmed today.");
...
const recommendationSort = (a, b) => (urgencyRank[b.urgency ?? "Monitor"] - urgencyRank[a.urgency ?? "Monitor"]) ||
...
export const getRecommendations = (filters = {}) => {
    ...
    if (filters.riskLevel)
        candidates = candidates.filter((sku) => sku.riskLevel === filters.riskLevel);
    if (filters.category)
        candidates = candidates.filter((sku) => sku.category === filters.category);
    ...
    if (filters.supplierId)
        recommendations = recommendations.filter((item) => item.recommendedSupplier?.supplierId === filters.supplierId);
    if (filters.urgency)
        recommendations = recommendations.filter((item) => item.urgency === filters.urgency);
    if (filters.limit)
        recommendations = recommendations.slice(0, filters.limit);
    return recommendations;
};
export const getRecommendationBySku = (skuId) => {
    const sku = getRiskBySku(skuId);
    if (!sku)
        return undefined;
    return buildRecommendation(sku);
};
export const generatePo = (options) => {
