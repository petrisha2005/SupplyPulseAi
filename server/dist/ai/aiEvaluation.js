...
export const evaluateCopilotResponse = ({ answer, evidence, executiveBriefing }) => {
    const issues = [];
    if (!evidence.length) {
        return {
            confidenceScore: 0,
    ...
    for (const evidenceItem of evidence) {
        if (evidenceItem.id && normalizedAnswer.includes(evidenceItem.id.toLowerCase()))
            referencedEvidenceIds.add(evidenceItem.id);
        if (normalizedAnswer.includes(evidenceItem.source.toLowerCase()) && evidenceItem.id)
            referencedEvidenceIds.add(evidenceItem.id);
    }
    ...
    const evidenceIds = evidence.map((item) => item.id).filter((id) => Boolean(id));
    ...
    if (!evidenceIds.length)
        issues.push("Response evidence does not include stable identifiers for citation coverage.");
    if (!executiveBriefing)
        issues.push("Executive briefing was not generated; response uses basic evidence presentation.");
    if (evidenceCoverage < 1)
        issues.push("Some available evidence was not explicitly referenced by the response.");
