...
forecastRouter.get("/compare", (req, res) => {
  ...
  if (!skuIds.length) return res.status(400).json({ error: "skuIds query parameter is required. Example: /api/forecast/compare?skuIds=SKU1,SKU2" });
  if (skuIds.length > 3) return res.status(400).json({ error: "Compare supports up to 3 SKUs." });
  const result = compareForecasts(skuIds);
  if (result.forecasts.length !== skuIds.length) return res.status(404).json({ error: "One or more SKU forecasts were not found.", foundSkuIds: result.forecasts.map((forecast) => forecast.skuId) });
