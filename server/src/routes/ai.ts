...
aiRouter.post("/risk-investigation", async (req, res) => {
  ...
  if (!input) return res.status(400).json({ error: "skuId must be a non-empty string" });

  const investigation = await investigateRisk(input.skuId);
  if (!investigation) return res.status(404).json({ error: "SKU not found" });
...
aiRouter.post("/sku-investigation", async (req, res) => {
  ...
  if (!input) return res.status(400).json({ error: "skuId and question must be non-empty strings; question must be 500 characters or fewer" });
