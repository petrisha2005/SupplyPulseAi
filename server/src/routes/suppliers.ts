...
suppliersRouter.get("/compare", (req, res) => {
  ...
  if (!ids.length) return res.status(400).json({ error: "ids query parameter is required. Example: /api/suppliers/compare?ids=SUP-BLR,SUP-SUR" });
  if (ids.length > 3) return res.status(400).json({ error: "Compare supports up to 3 suppliers." });
  const result = compareSuppliers(ids);
  if (result.suppliers.length !== ids.length) return res.status(404).json({ error: "One or more suppliers were not found.", foundIds: result.suppliers.map((supplier) => supplier.supplierId ?? supplier.id) });
  return res.json(result);
});

suppliersRouter.get("/:supplierId/dependencies", (req, res) => {
  const result = getSupplierDependencies(req.params.supplierId);
  if (!result) return res.status(404).json({ error: "Supplier dependencies not found" });
  return res.json(result);
});
...
suppliersRouter.get("/:skuId", (req, res) => {
  ...
  if (supplier) return res.json(getSupplierDependencies(supplier.supplierId));
  const sku = getRiskBySku(req.params.skuId);
  if (!sku) return res.status(404).json({ error: "Supplier or SKU not found" });
