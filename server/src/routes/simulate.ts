...
import type { Channel } from "@supplypulse/shared";
...
simulateRouter.post("/flash-sale", (req, res) => {
  ...
  const channel = typeof req.body?.channel === "string" ? req.body.channel as Channel : "Amazon";
...
simulateRouter.post("/stockout", (req, res) => {
  ...
  const channel = String(req.body?.channel ?? "Amazon") as Channel;
