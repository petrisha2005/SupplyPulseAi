import type { SeedSupplier, Supplier, SupplierCompareResponse, SupplierDependenciesResponse } from "@supplypulse/shared";
...
export const toLegacySupplier = (supplierId: string): Supplier => {
...
const supplierRiskLevel = (score: number): Supplier["supplierRiskLevel"] => {
...
const serviceHealth = (level: Supplier["supplierRiskLevel"]) => {
  if (level === "Healthy") return "Reliable today";
  if (level === "Watch") return "Watch closely";
  if (level === "Risky") return "Delay pressure";
  return "Avoid for critical SKUs";
};

const recommendedUsage = (level: Supplier["supplierRiskLevel"], delayDays: number, criticalSkus: number): Supplier["recommendedUsage"] => {
  if (level === "Critical" || delayDays >= 4) return "Avoid today";
  if (level === "Risky" || criticalSkus >= 4) return "Backup supplier only";
  if (level === "Watch") return "Planned replenishment";
  return "Urgent orders";
};

const insight = (supplier: SeedSupplier, level: Supplier["supplierRiskLevel"], highRiskSkus: number, criticalSkus: number) => {
  if (level === "Healthy") return "Healthy supplier. Good for planned replenishment and urgent dispatch when stock cover is tight.";
  if (level === "Watch") return `Watch closely: ${highRiskSkus} high-risk SKUs depend on this supplier.`;
  if (level === "Risky") return `Risky for urgent orders because recent delay pressure is ${supplier.lastDelayDays} days and ${criticalSkus} critical SKUs depend on it.`;
  return "Avoid for critical SKUs today; choose a faster alternate supplier.";
};

const scoreSupplier = (supplier: SeedSupplier, linkedRevenueAtRisk: number, criticalSkus: number) => {
...
export const getSupplierScorecards = (): Supplier[] => {
...
export const getSupplierDependencies = (supplierId: string): SupplierDependenciesResponse | undefined => {
...
export const compareSuppliers = (ids: string[]): SupplierCompareResponse => {
  const suppliers = getSupplierScorecards().filter((supplier) => ids.includes(supplier.supplierId ?? supplier.id));
  const bestForUrgentReorder = [...suppliers].sort((a, b) =>
    (a.avgLeadDays ?? a.averageLeadTime) - (b.avgLeadDays ?? b.averageLeadTime) ||
    b.reliabilityScore - a.reliabilityScore ||
    (a.supplierRiskScore ?? 100) - (b.supplierRiskScore ?? 100)
  )[0];
...
export const supplierSummaryText = (supplier: Supplier) =>
  `${supplier.name}: ${supplier.supplierRiskLevel} risk, ${supplier.avgLeadDays ?? supplier.averageLeadTime} day lead time, ${supplier.onTimeDeliveryPct}% on-time, ${compactRupee(supplier.totalRevenueAtRiskLinked ?? 0)} revenue linked.`;
