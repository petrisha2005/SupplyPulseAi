import type { PipelineRun, PipelineStage, PipelineStatus } from "@supplypulse/shared";
...
const stageMessage = (name: string, rows: number) => {
  if (name === "Marketplace ingestion") return `Ingested ${rows.toLocaleString("en-IN")} channel sales and stock rows.`;
  if (name === "Data cleaning") return "Normalized returns, missing ERP quantities, and duplicate marketplace rows.";
  if (name === "Channel inventory fusion") return "Merged Amazon, Shopify, Meesho, Flipkart, and ERP stock positions.";
  if (name === "Forecast generation") return "Generated 7-day and 30-day SKU demand forecasts.";
  if (name === "Risk scoring") return "Recomputed explainable stockout risk scores.";
  if (name === "Recommendation generation") return "Generated reorder quantities and supplier recommendations.";
  if (name === "Alert generation") return "Generated operational alert signals and deduped pending alerts.";
  return "Dashboard data refreshed.";
};

export const getPipelineStatus = () => lastStatus;

export const runPipeline = (): PipelineRun => {
  ...
  const stages: PipelineStage[] = stageTemplates.map((name, index) => {
