import type { DashboardResponse, ExecutiveReportResponse, RiskLevel } from "@supplypulse/shared";
...
const countByRisk = (level: RiskLevel, risks: ReturnType<typeof getRiskList>) => risks.filter((sku) => sku.riskLevel === level).length;
...
export const getExecutiveSummaryReport = (): ExecutiveReportResponse => {
  ...
  } satisfies DashboardResponse & ExecutiveReportResponse["dashboardSummary"];
