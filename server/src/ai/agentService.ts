...
import type { MorningBriefContent, MorningBriefResponse, SkuInvestigationEvidence, SkuInvestigationResponse } from "@supplypulse/shared";
import { riskInvestigationEvidenceKeys, type RiskInvestigationContext, type RiskInvestigationEvidenceKey, type RiskInvestigationResponse } from "./schemas.js";
import { getMorningBriefContext, getRiskInvestigationContext, getSkuInvestigationContext } from "./supplyPulseTools.js";

const evidenceLabels: Record<RiskInvestigationEvidenceKey, string> = {
...
const fallbackInvestigation = (context: RiskInvestigationContext) => ({
  ...
  evidenceKeys: riskInvestigationEvidenceKeys.filter((key) => {
    if (key === "forecast") return Boolean(context.forecast);
    if (key === "supplier_dependencies") return Boolean(context.supplierDependencies);
    if (key === "reorder_recommendation") return Boolean(context.recommendation);
    if (key === "alerts") return context.alerts.length > 0;
    return true;
  }),
  limitations: ["Gemini is disabled or unavailable; this explanation is generated from deterministic SupplyPulse engines."]
});

export const investigateRisk = async (skuId: string): Promise<RiskInvestigationResponse | undefined> => {
  ...
  const source = geminiInvestigation ? "gemini" as const : "deterministic-fallback" as const;
...
const fallbackMorningBrief = (context: ReturnType<typeof getMorningBriefContext>): MorningBriefContent => {
  const priorities = context.topRiskSkus.slice(0, 3).map((sku) => {
    ...
    return {
      ...
      severity: sku.riskLevel === "Critical" ? "critical" as const : sku.riskLevel === "High" ? "high" as const : sku.riskLevel === "Medium" ? "medium" as const : "low" as const
    };
  });
  const overallHealth: MorningBriefContent["overallHealth"] = context.pipeline.healthLabel === "Healthy" && !priorities.some((priority) => priority.severity === "critical")
    ? "Healthy"
...
export const generateMorningBrief = async (): Promise<MorningBriefResponse> => {
...
const investigationEvidence = (context: RiskInvestigationContext): SkuInvestigationEvidence[] => {
  const evidence: SkuInvestigationEvidence[] = [{ source: "Risk Engine", entity: context.risk.skuId }];
  if (context.forecast) evidence.push({ source: "Forecast Engine", entity: context.risk.skuId });
  if (context.supplierDependencies) evidence.push({ source: "Supplier Engine", entity: context.supplierDependencies.supplier.supplierId ?? context.risk.supplierId });
  if (context.recommendation) evidence.push({ source: "Recommendation Engine", entity: context.recommendation.skuId });
  return evidence;
};

const fallbackSkuInvestigation = (context: RiskInvestigationContext, question: string): Omit<SkuInvestigationResponse, "skuId" | "source" | "evidence"> => {
...
export const investigateSku = async (skuId: string, question: string): Promise<SkuInvestigationResponse | undefined> => {
