...
const supplierForCategory = (category: string, index: number) => {
...
const categoryChannelMix: Record<string, Partial<Record<Channel, number>>> = {
...
const channelSplit = (category: string, index: number): Record<Channel, number> => {
  ...
  return Object.fromEntries(channels.map((channel) => [channel, Number((raw[channel] / total).toFixed(2))])) as Record<Channel, number>;
};

const salesHistory = (index: number, price: number): SalesPoint[] => {
...
export const buildInventory = (): InventorySku[] => {
...
const labelRisk = (score: number): RiskLevel => {
...
export const scoreRisk = (sku: InventorySku): RiskSku => {
  const coverPressure =
    sku.daysOfCover <= 1.5 ? 38 :
  ...
  const channelHotspot = (Object.entries(sku.channelDemandSplit) as Array<[Channel, number]>).sort((a, b) => b[1] - a[1])[0][0];
  return {
    ...sku,
    riskScore: score,
...
export const getDashboard = (): DashboardResponse => {
...
const forecastPoints = (sku: InventorySku, days: number): SalesPoint[] => {
...
export const getForecast = (skuId: string): ForecastResponse | undefined => {
