...
import { ResponseInsightCards } from "./ResponseInsightCards";
  ...
  return (
    <section className="space-y-4">
      {messages.map((message) => message.role === "user" ? (
        ...
        <article key={message.id} className="rounded-[1.5rem] border border-white/75 bg-white/70 p-4 shadow-xl shadow-slate-950/5 backdrop-blur dark:border-white/10 dark:bg-slate-900/75"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-teal-700 text-white shadow-lg shadow-teal-900/15"><Bot size={18} /></span><div><p className="text-xs font-black uppercase tracking-wide text-teal-700 dark:text-teal-300">SupplyPulse intelligence</p><p className="mt-1 text-sm font-semibold text-slate-500 dark:text-slate-400">Evidence-backed executive decision brief</p></div></div><div className="flex flex-col items-end gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${message.response.generatedBy === "gemini" ? "bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-200" : "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-200"}`}>{message.response.generatedBy === "gemini" ? "AI reasoning" : "Deterministic fallback"}</span><ConfidenceBadge confidence={message.response.metadata?.confidenceScore ?? message.response.confidence} grounding={message.response.metadata?.groundingScore} /></div></div>
          <ResponseInsightCards answer={message.response.answer} />
